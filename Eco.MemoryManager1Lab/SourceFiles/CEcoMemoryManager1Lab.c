...
static int16_t ECOCALLMETHOD CEcoMemoryManager1Lab_623E1838_Init(/* in */ IEcoMemoryManager1Ptr_t me, /* in */ voidptr_t startAddress, /* in */ uint32_t size) {
    ...
    align = (uint32_t)sizeof(voidptr_t);
    addr = (uint64_t)(uintptr_t)startAddress;

    if ((addr & (align - 1)) != 0) {
        uint64_t aligned = (addr + (align - 1)) & ~((uint64_t)(align - 1));
        delta = (uint32_t)(aligned - addr);

        if (size <= delta) {
            return -1;
        }

        startAddress = (voidptr_t)(uintptr_t)aligned;
        size -= delta;
    }

    if (size <= sizeof(HeapBlock)) {
        return -1;
    }

    pCMe->m_pHeapStart = (char_t*)startAddress;
    pCMe->m_pHeapEnd = (char_t*)startAddress + size;
    pCMe->m_pHeapBreak = pCMe->m_pHeapStart;

    pCMe->m_iHeapSize = size;
    pCMe->m_iAllocSize = 0;
    pCMe->m_iBlockNum = 0;

    first = (HeapBlock*)startAddress;
    first->prev = 0;
    first->next = 0;
    first->freeFlag = 1;
